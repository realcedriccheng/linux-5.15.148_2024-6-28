...
#ifdef QWJ_DEBUG_FS
static void procfs_update_general_status(struct f2fs_sb_info *sbi)
{
	struct f2fs_stat_info *si = F2FS_STAT(sbi);
	struct f2fs_super_block *raw_super = F2FS_RAW_SUPER(sbi);
	int i;
 ...
 * This function calculates memory footprint.
 */
static void procfs_update_mem_info(struct f2fs_sb_info *sbi)
{
	struct f2fs_stat_info *si = F2FS_STAT(sbi);
	int i;
...
int procfs_stat_show(struct seq_file *s, void *v)
{
	...
	raw_spin_lock_irqsave(&f2fs_stat_lock, flags);
	list_for_each_entry(si, &f2fs_stat_list, stat_list) {
		procfs_update_general_status(si->sbi);
