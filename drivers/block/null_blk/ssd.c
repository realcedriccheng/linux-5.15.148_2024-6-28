...
void ssd_init_params(struct nullb_device *dev, struct ssdparams *spp, uint64_t capacity, uint32_t nparts)
{
	...
	if (dev->ufs) {
		...
		// capacity = capacity / 9 * 10;
		capacity = 8724152320; //8320 * 1024 * 1024
	...
	// printk("spp->pgs_per_blk = %llu, ")

	spp->write_unit_size = WRITE_UNIT_SIZE;

#ifdef USER_SET_LAT
	spp->pg_4kb_rd_lat[CELL_TYPE_LSB] = dev->read_nsec;
	spp->pg_4kb_rd_lat[CELL_TYPE_MSB] = dev->read_nsec;//NAND_4KB_READ_LATENCY_MSB
	spp->pg_4kb_rd_lat[CELL_TYPE_CSB] = dev->read_nsec;
	spp->pg_rd_lat[CELL_TYPE_LSB] = dev->read_nsec + 6000;
	spp->pg_rd_lat[CELL_TYPE_MSB] = dev->read_nsec + 6000;
	spp->pg_rd_lat[CELL_TYPE_CSB] = dev->read_nsec + 6000;
	spp->pg_wr_lat = dev->write_nsec;
	spp->blk_er_lat = dev->erase_nsec;
#else
	spp->pg_4kb_rd_lat[CELL_TYPE_LSB] = NAND_4KB_READ_LATENCY_LSB;
	spp->pg_4kb_rd_lat[CELL_TYPE_MSB] = NAND_4KB_READ_LATENCY_MSB;//NAND_4KB_READ_LATENCY_MSB
	...
	// spp->pg_4kb_rd_lat[CELL_TYPE_LSB] = 10000;
	// spp->pg_4kb_rd_lat[CELL_TYPE_MSB] = 10000;//NAND_4KB_READ_LATENCY_MSB
	// spp->pg_4kb_rd_lat[CELL_TYPE_CSB] = 10000;
	// spp->pg_rd_lat[CELL_TYPE_LSB] = 10000;
	// spp->pg_rd_lat[CELL_TYPE_MSB] = 10000;
	// spp->pg_rd_lat[CELL_TYPE_CSB] = 10000;
	// spp->pg_wr_lat = 10000;
	// spp->blk_er_lat = 10000;
	// spp->max_ch_xfer_size = MAX_CH_XFER_SIZE;

	// spp->fw_4kb_rd_lat = 10000;
	// spp->fw_rd_lat = 10000;
	// spp->fw_ch_xfer_lat = FW_CH_XFER_LATENCY;
	...
	// spp->ch_bandwidth = 100;
	// spp->pcie_bandwidth = 100;
...
#ifdef SLC_CACHE
void ssd_init_params_slc(struct nullb_device *dev, struct ssdparams *spp, uint64_t capacity, uint32_t nparts)
{
	uint64_t blk_size, total_size;
...
#ifdef USER_SET_LAT
	spp->pg_4kb_rd_lat[CELL_TYPE_LSB] = dev->read_nsec / 2;
	spp->pg_4kb_rd_lat[CELL_TYPE_MSB] = dev->read_nsec / 2;//NAND_4KB_READ_LATENCY_MSB
	spp->pg_4kb_rd_lat[CELL_TYPE_CSB] = dev->read_nsec / 2;
	spp->pg_rd_lat[CELL_TYPE_LSB] = (dev->read_nsec + 6000) / 2;
	spp->pg_rd_lat[CELL_TYPE_MSB] = (dev->read_nsec + 6000) / 2;
	spp->pg_rd_lat[CELL_TYPE_CSB] = (dev->read_nsec + 6000) / 2;
	spp->pg_wr_lat = dev->write_nsec / 3;
	spp->blk_er_lat = dev->erase_nsec;
#else
	spp->pg_4kb_rd_lat[CELL_TYPE_LSB] = NAND_4KB_READ_LATENCY_LSB;
	spp->pg_4kb_rd_lat[CELL_TYPE_MSB] = NAND_4KB_READ_LATENCY_MSB;//NAND_4KB_READ_LATENCY_MSB
...
// 	spp->wl = 0;
	spp->ecc_corr_str = 50;
	spp->denominator = 19808259324075;
	spp->epsilon = 29345569369;
	spp->alpha = 10228501;
	spp->beta = 1243958685;
	spp->gamma = 129105;
	spp->k = 2;
...
static void ssd_init_ch(struct ssd_channel *ch, struct ssdparams *spp)
{
	...
	spin_lock_init(&ch->channel_lock);
...
void ssd_init(struct ssd *ssd, struct ssdparams *spp)
{
	uint32_t i;
	/* copy spp */
	ssd->sp = *spp;
...
uint64_t ssd_advance_pcie(struct ssd *ssd, uint64_t request_time, uint64_t length)//qwj
{
	struct channel_model *perf_model = ssd->pcie->perf_model;
	// uint64_t ret;
	// spin_lock(&ssd->pcie->pcie_lock);
#ifdef NVMEV_CHANNEL_LAT
	return chmodel_request_pcie(perf_model, request_time, length);
#else
	return request_time;
#endif
	// spin_unlock(&ssd->pcie->pcie_lock);
	// return ret;
	// return request_time;//代替
}
...
uint64_t ssd_advance_nand(struct ssd *ssd, struct nand_cmd *ncmd) //qwj
{
	int c = ncmd->cmd;
	...
	case NAND_READ:
		...
		// NVMEV_INFO("[%s] nand_stime 0x%llx nand_etime 0x%llx cmd_stime 0x%llx\n",
			    // __func__, nand_stime, nand_etime, cmd_stime);
		/* read: then data transfer through channel */
		chnl_stime = nand_etime;
#ifdef NVMEV_CHANNEL_LAT
		while (remaining) {
			xfer_size = min(remaining, (uint64_t)spp->max_ch_xfer_size);
			// chnl_etime = chnl_stime;//代替
