...
#ifdef MAP_ENTRY_LRU
static void add_node_to_head(struct LRU_cache *cache, struct LRU_node *node)
{
...
static struct LRU_node * remove_node(struct LRU_cache *cache, struct LRU_node *node)
{
	BUG_ON(!node);
	if (node->prev != NULL) {
...
static void init_cache(struct nullb_device *dev, struct conv_ftl *conv_ftl)
{
	int i;
	struct ssdparams *spp = &conv_ftl->ssd->sp;

	conv_ftl->maptbl_cache = vmalloc(sizeof(struct LRU_node *) * spp->tt_pgs);
	for (i = 0; i < spp->tt_pgs; i++)
		conv_ftl->maptbl_cache[i] = NULL;
	
	conv_ftl->cache = kmalloc(sizeof(struct LRU_cache), GFP_KERNEL);
	conv_ftl->cache->size = 0;
	conv_ftl->cache->capacity = CACHE_SIZE;
	conv_ftl->cache->head = NULL;
	conv_ftl->cache->tail = NULL;
	conv_ftl->map_entry_nr_per_pg = dev->map_entry_nr_per_pg;
	mutex_init(&conv_ftl->cache_lock);
}

static void remove_cache(struct conv_ftl *conv_ftl)
{
	struct LRU_node *p = conv_ftl->cache->head, *q;
	while (p != NULL) {
		q = p;
		p = p->next;
		kfree(q);
	}
	kfree(conv_ftl->cache);
	vfree(conv_ftl->maptbl_cache);
}

static bool search_l2p_table(struct conv_ftl *conv_ftl, uint64_t lpn)
{
	uint64_t start_lpn = lpn / conv_ftl->map_entry_nr_per_pg; //cache size = conv_ftl->map_entry_nr_per_pg * 4B * CACHE_SIZE
	struct LRU_node *node;
	struct LRU_cache *cache = conv_ftl->cache;
	// printk("conv_ftl->map_entry_nr_per_pg = %llu\n",conv_ftl->map_entry_nr_per_pg);
	mutex_lock(&conv_ftl->cache_lock);
	node = conv_ftl->maptbl_cache[start_lpn];
	if(node == NULL) {
		// add_node_to_head
		node = kmalloc(sizeof(struct LRU_node), GFP_KERNEL);
		node->lpn = start_lpn;
		if (cache->size < cache->capacity) {
			add_node_to_head(cache, node);
		}
		else {
			struct LRU_node *removed_node = remove_node(cache, cache->tail);
			conv_ftl->maptbl_cache[removed_node->lpn] = NULL;
			BUG_ON(!removed_node);
			if (removed_node)
				kfree(removed_node);
			add_node_to_head(cache, node);
		}
		conv_ftl->maptbl_cache[start_lpn] = node;
		// printk("miss");
		mutex_unlock(&conv_ftl->cache_lock);
		return false;
	}
	else {
...
static void conv_init_ftl(struct nullb_device *dev, struct conv_ftl *conv_ftl, struct convparams *cpp, struct ssd *ssd)
{
	/*copy convparams*/
	conv_ftl->cp = *cpp;
	...
	conv_ftl->dev = dev;//for gc count

	/* initialize maptbl */
	init_maptbl(conv_ftl); // mapping table

#ifdef MAP_ENTRY_LRU
	init_cache(dev, conv_ftl); // mapping cache
#endif
	/* initialize rmap */
...
#ifdef MAP_ENTRY_LRU
	remove_cache(conv_ftl);
#endif
}

static void conv_init_params(struct convparams *cpp)
{
	cpp->op_area_pcent = 0.1;
...
void conv_init_namespace(struct nullb_device *dev, uint64_t size)
{
	...
	ssd_init_params(dev, &spp, size, nr_parts);
	conv_init_params(&cpp);

	conv_ftls = kmalloc(sizeof(struct conv_ftl) * nr_parts, GFP_KERNEL);
	if (conv_ftls){
		dev->conv_ftls = conv_ftls;
		printk("dev->conv_ftls = %llu\n",dev->conv_ftls);
	}
	else{
		NVMEV_ERROR("conv_init_namespace fail!!!");
		return;
	}
	...
	for (i = 0; i < nr_parts; i++) {
		...
		ssd_init(ssd, &spp);
		conv_init_ftl(dev, &conv_ftls[i], &cpp, ssd);
		mutex_init(&conv_ftls[i].write_lock);
	}
...
{
	struct conv_ftl *conv_ftls = dev->conv_ftls;
	const uint32_t nr_parts = SSD_PARTITIONS;
	uint32_t i;
...
static void mark_page_valid(struct conv_ftl *conv_ftl, struct ppa *ppa)
{
	...
	// 	printk("ppa: %llu %llu %llu %llu %llu || status: %u  || %llu %llu %llu %llu\n",ppa->g.pg,ppa->g.blk,ppa->g.pl,ppa->g.lun,ppa->g.ch,pg->status,spp->pgs_per_blk, spp->blks_per_lun, spp->luns_per_ch, spp->nchs);
...
static int do_gc(struct conv_ftl *conv_ftl, bool force)
{
	...
	conv_ftl->dev->gc_move_pages += victim_line->vpc;//for gc count
	printk("GC-ing line:%d,ipc=%d(%d),victim=%d,full=%d,free=%d\n", ppa.g.blk,
		    victim_line->ipc, victim_line->vpc, conv_ftl->lm.victim_line_cnt,
		    conv_ftl->lm.full_line_cnt, conv_ftl->lm.free_line_cnt);
...
{
	struct conv_ftl *conv_ftls = cmd->nq->dev->conv_ftls;
	...
	struct nand_cmd srd = {
		...
		.cmd = NAND_READ,
		.stime = nsecs_start,
		.interleave_pci_dma = true,//true
	};
	// printk("qwj-conv_read\n");
	if (conv_ftl->ssd){
		spp = &conv_ftl->ssd->sp;
	...
	// printk("ufs-read: start_lpn: %llu nr_lba: %llu end_lpn: %llu\n",start_lpn,nr_lba,end_lpn);
	mutex_lock(&conv_ftl->write_lock);
	// for (i = 0; (i < nr_parts) && (start_lpn <= end_lpn); i++, start_lpn++) {
		// conv_ftl = &conv_ftls[1];
		xfer_size = 0;
		prev_ppa = get_maptbl_ent(conv_ftl, start_lpn);

		/* normal IO read path */
		for (lpn = start_lpn; lpn <= end_lpn; lpn += nr_parts) {
			uint64_t local_lpn;
			struct ppa cur_ppa;
			// uint64_t cur_time = cpu_clock(1);
			// NVMEV_INFO("[%s] lpn %llu cur_time 0x%llx cmd->nsecs_start 0x%llx\n",
			//     __func__, lpn, cur_time, cmd->nsecs_start);
			local_lpn = lpn;
			...
			if (!mapped_ppa(&cur_ppa) || !valid_ppa(conv_ftl, &cur_ppa)) {
				...
				// NVMEV_INFO("[%s] lpn %llu xfer_size %lu !mapped_ppa(&cur_ppa) || !valid_ppa(conv_ftl, &cur_ppa)\n",
			    // __func__, lpn,xfer_size);
				continue;
			}
#ifdef MAP_ENTRY_LRU
			if (!search_l2p_table(conv_ftl, local_lpn)) {
				// printk("miss, srd.stime:%lld", srd.stime);
				// printk("miss read");
				srd.stime += spp->pg_rd_lat[1] * 3;
			}
#endif
			...
			if (mapped_ppa(&prev_ppa) &&
				...
				xfer_size += spp->pgsz;
				// NVMEV_INFO("[%s] lpn %llu xfer_size %lu ---%lu %lu %lu %lu %lu--- is_same_flash_page\n",
			    // __func__, lpn, xfer_size, cur_ppa.g.ch, cur_ppa.g.lun, cur_ppa.g.pl,cur_ppa.g.blk, cur_ppa.g.pg);
				continue;
			}
	...
	if (srd.interleave_pci_dma == false) {
		nsecs_completed = ssd_advance_pcie(conv_ftl->ssd, nsecs_latest, nr_lba * spp->secsz);
		nsecs_latest = (nsecs_completed > nsecs_latest) ? nsecs_completed : nsecs_latest;
	}
	mutex_unlock(&conv_ftl->write_lock);
				    ...
				    unsigned int nr_sectors)
{
	...
	// 	return false;
	// printk("ufs-write: start_lpn: %llu nr_lba: %llu end_lpn: %llu\n",start_lpn,nr_lba,end_lpn);
	mutex_lock(&conv_ftl->write_lock);

	nsecs_latest =
		ssd_advance_write_buffer(conv_ftl->ssd, cmd->nsecs_start, LBA_TO_BYTE(nr_lba));
	...
	for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
		...
		// NVMEV_INFO("[%s] lpn %llu ppa.g.pg %u\n",
			    // __func__, lpn, ppa.g.pg);
		// if(is_same_flash_page(conv_ftl, ppa, prev_ppa)){
		// 	// NVMEV_INFO("[%s] lpn %llu is_same_flash_page(conv_ftl, ppa, prev_ppa)\n",
		// 	//     __func__, lpn);
		// 	count++;
		// }else
		// 	count=0;
		// if (count > 3)
		// 	NVMEV_INFO("[%s] count > 3 lpn %llu is_same_flash_page(conv_ftl, ppa, prev_ppa)\n",
		// 	    __func__, lpn);
		// prev_ppa = ppa;//qwj add
		/* need to advance the write pointer here */
		advance_write_pointer(conv_ftl, USER_IO);
		// printk("ppa: %llu %llu %llu %llu %llu || lpn: %llu nr_lba: %llu end_lpn: %llu|| %llu %llu %llu %llu\n",ppa.g.pg,ppa.g.blk,ppa.g.pl,ppa.g.lun,ppa.g.ch,lpn,nr_lba,end_lpn,spp->pgs_per_blk, spp->blks_per_lun, spp->luns_per_ch, spp->nchs);
		...
		// else if ( flag == 0 && ((end_lpn - lpn) < (uint64_t)(spp->pgs_per_oneshotpg))){
		// 	swr.ppa = &ppa;
