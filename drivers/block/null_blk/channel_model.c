...
uint64_t chmodel_request(struct channel_model *ch, uint64_t request_time, uint64_t length)//qwj
{
	...
	pos = (ch->head + request_time_offs) % NR_CREDIT_ENTRIES; //从pos才开始消耗，处理请求
	...
	// printk("total_latency = %llu\n",total_latency);

	return request_time + total_latency;
}


uint64_t chmodel_request_pcie(struct channel_model *ch, uint64_t request_time, uint64_t length)//qwj
{
	uint64_t cur_time = __get_wallclock();
	uint32_t pos, next_pos;
	uint32_t remaining_credits, consumed_credits;
