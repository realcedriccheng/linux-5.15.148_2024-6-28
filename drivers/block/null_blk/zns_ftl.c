...
static void zns_init_ftl(struct zns_ftl *zns_ftl, struct znsparams *zpp, struct ssd *ssd)
{
	*zns_ftl = (struct zns_ftl){
	...
	NVMEV_INFO("========init zufs ftl========\n");
	// __init_descriptor(zns_ftl);
	// __init_resource(zns_ftl);
}

#ifdef SLC_CACHE
static void slc_init_params(struct slcparams *slcpp, struct ssdparams *spp, uint64_t capacity)
{
	int i;
	*slcpp = (struct slcparams){
		.zone_size = ZONE_SIZE,
		.nr_zones = capacity / ZONE_SIZE,
		.dies_per_zone = DIES_PER_ZONE,
		.secs_per_zone = ZONE_SIZE / spp->secsz,
		// .active_zones = 0,

		// .zone_map = kmalloc(sizeof(int) * slcpp->nr_zones, GFP_KERNEL),
		// .bit_map = kmalloc(sizeof(bool) * slcpp->nr_zones * slcpp->secs_per_zone, GFP_KERNEL),
		// .used_sectors = kmalloc(sizeof(int) * slcpp->nr_zones, GFP_KERNEL),
	...
	atomic_set(&slcpp->active_zones, 0);

	slcpp->zone_map = kmalloc(sizeof(int) * slcpp->nr_zones, GFP_KERNEL);
	slcpp->bit_map = kmalloc(sizeof(bool) * slcpp->nr_zones * slcpp->secs_per_zone, GFP_KERNEL);
	slcpp->used_sectors = kmalloc(sizeof(int) * slcpp->nr_zones, GFP_KERNEL);
	slcpp->cmd_tlc = kmalloc(sizeof(struct nullb_cmd) * slcpp->nr_zones, GFP_KERNEL);

	for(i = 0; i < slcpp->nr_zones; i++) {
		// printk("init zone_map");
		slcpp->zone_map[i] = -1;
		slcpp->used_sectors[i] = 0;
	}
		
	for(i = 0; i < slcpp->nr_zones * slcpp->secs_per_zone; i++)
		slcpp->bit_map[i] = false;

	NVMEV_ASSERT((capacity % slcpp->zone_size) == 0);
	/* It should be 4KB aligned, according to lpn size */
	NVMEV_ASSERT((slcpp->zone_size % spp->pgsz) == 0);

	NVMEV_INFO("zone_size=%u(Byte),%u(MB), # zones=%d # die/zone=%d \n", slcpp->zone_size,
		   BYTE_TO_MB(slcpp->zone_size), slcpp->nr_zones, slcpp->dies_per_zone);
}

static void slc_init_cache(struct slc_cache *slc_cache, struct slcparams *slcpp, struct ssd *ssd)
{
	*slc_cache = (struct slc_cache){
		.slcp = *slcpp, /*copy znsparams*/

		.ssd = ssd,
		// .storage_base_addr = mapped_addr,
	};
	NVMEV_INFO("========init SLC cache ftl========\n");
	// __init_descriptor(zns_ftl);
	// __init_resource(zns_ftl);
}
#endif
...
void zns_init_namespace(struct nullb_device *dev, uint64_t size)
{
	struct ssd *ssd;
	struct zns_ftl *zns_ftl;
#ifdef SLC_CACHE
	struct slc_cache *slc_cache;
	struct slcparams slcpp;
	struct ssd *ssd_slc;
	struct ssdparams spp_slc;
#endif
	...
	zns_init_ftl(zns_ftl, &zpp, ssd);
	mutex_init(&zns_ftl->write_lock);
	...
	// };
#ifdef SLC_CACHE
	ssd_init_params_slc(dev, &spp_slc, ZONE_SIZE * dev->slc_zone_number, nr_parts);
	ssd_slc = kmalloc(sizeof(struct ssd), GFP_KERNEL);
	ssd_init(ssd_slc, &spp_slc);

	slc_cache = kmalloc(sizeof(struct slc_cache) * nr_parts, GFP_KERNEL);
	slc_init_params(&slcpp, &spp_slc, ZONE_SIZE * dev->slc_zone_number);
	slc_init_cache(slc_cache, &slcpp, ssd_slc);

	spin_lock_init(&slc_cache->write_lock);
	
	dev->slc_cache = slc_cache;
#endif
	return;
}

void zns_remove_namespace(struct nullb_device *dev)
{
	struct zns_ftl *zns_ftl = dev->zns_ftl;
#ifdef SLC_CACHE
	struct slc_cache *slc_cache = dev->slc_cache;

	if(slc_cache){
		ssd_remove(slc_cache->ssd);

		kfree(slc_cache->slcp.bit_map);
		kfree(slc_cache->slcp.zone_map);
		kfree(slc_cache->slcp.used_sectors);
		kfree(slc_cache->slcp.cmd_tlc);
		kfree(slc_cache->ssd);
		kfree(slc_cache);
	}
#endif
